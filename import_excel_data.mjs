...
async function run() {
  ...
  // We need host_id and property_id. We can get it from 'properties' table.
  const { data: properties, error: pError } = await supabase.from('properties').select('id, host_id').limit(1);
  
  if (pError || !properties || properties.length === 0) {
    console.error('Error fetching property info. Ensure user is logged in or properties exist.', pError);
    process.exit(1);
  }
  
  const propertyId = properties[0].id;
  const hostId = properties[0].host_id;

  console.log(`Using Property ID: ${propertyId}, Host ID: ${hostId}`);

  // Let's clear the bookings table first just to be completely clean
  // But wait, the user said they added bookingdate column. Did they delete the bookings? 
  // We will delete all existing bookings to prevent duplicates.
  console.log('Clearing existing bookings...');
  await supabase.from('bookings').delete().neq('id', '00000000-0000-0000-0000-000000000000');

  console.log('Preparing insert payload from DUMMY_BOOKINGS...');
  const rowsToInsert = DUMMY_BOOKINGS.map(b => {
    // Find matching booking date from our map
    let bdStr = null;
    ...
    if (match) {
      ...
      // Generate a reasonable random date if not found in map
      const ciDate = new Date(b.checkIn + 'T12:00:00');
      const randDays = Math.floor(Math.random() * (70 - 15 + 1)) + 15;
  ...
  // Supabase might have limits on bulk insert size, but 200 rows is fine
  const { data, error } = await supabase.from('bookings').insert(rowsToInsert);
  
  if (error) {
    console.error('Error inserting data:', error);
  } else {
    console.log('Successfully inserted all records with booking dates!');
  }
}
