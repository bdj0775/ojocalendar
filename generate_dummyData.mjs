...
for (let i = 0; i < lines.length; i++) {
  ...
  const commissionWon = parseInt(commStr, 10);
  const commission = amount > 0 ? parseFloat((commissionWon / amount * 100).toFixed(1)) : 0;
...
// Ensure the format exactly matches dummyData.ts structure
let outTs = `import type { Booking, Maintenance } from '../types';

export const DUMMY_BOOKINGS: Booking[] = [\n`;

bookings.forEach(b => {
  outTs += `  { id: ${b.id}, guestName: '${b.guestName.replace(/'/g, "\\'")}', checkIn: '${b.checkIn}', checkOut: '${b.checkOut}', bookingDate: '${b.bookingDate}', guests: ${b.guests}, infants: ${b.infants}, nationality: '${b.nationality.replace(/'/g, "\\'")}', channel: '${b.channel.replace(/'/g, "\\'")}', status: '${b.status}', amount: ${b.amount}, commission: ${b.commission} },\n`;
});

outTs += `];

export const DUMMY_MAINTENANCE: Maintenance[] = [
  { id: 'm1', propertyId: 'prop1', startDate: '2026-03-02', endDate: '2026-03-04', label: 'ROOM MAINTENANCE' },
  { id: 'm2', propertyId: 'prop1', startDate: '2026-03-17', endDate: '2026-03-19', label: 'DEEP CLEANING' },
];
`;

fs.writeFileSync('src/utils/dummyData.ts', outTs, 'utf8');
console.log('Successfully written parsed data to src/utils/dummyData.ts');
