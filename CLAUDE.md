...
알고리즘·정확도·점검 절차·발전 로드맵은 전부 **`FORECAST.md`** 한 문서에 있습니다. **수정 전 필독.**

공식(픽업6): `예상 = 현재 OTB + (대상월 직전 6개 달력월 중 완료달들의 같은 시점
잔여픽업 평균)`, `[OTB, 100]` 클램프 + 진행 중인 달은 물리적 상한.

수정 시 지킬 것:
- 검증은 실제 훅을 실행하는 스크립트로만 (`run_hook_check.mjs` / `backtest_hook.mjs` /
  `monthly_review.mjs`). 공식을 재현한 별도 스크립트로 채택 판단 금지 — 오진한 전례 있음.
- 한 번에 하나씩 바꾸고 매번 전후 백테스트 비교.
- 윈도 정의("대상월 직전 6개 달력월 ∩ 완료달")를 바꾸지 말 것 — 근거는 FORECAST.md.
- 공식을 바꾸면 팝오버(`ForecastExplainer.tsx`) 문구도 함께 바꿀 것 (계산 과정을 그대로 노출함).
