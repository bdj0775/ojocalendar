...
`useDesktopStats.ts`의 `computeForecast()` 함수가 예상 점유율을 계산합니다.
변경 전에 반드시 **`FORECAST_SYSTEM.md`** 를 먼저 읽을 것.
(`FORECAST_ALGORITHM.md`·`FORECAST_DIAGNOSIS.md`는 2026-09 이전 구방식 기록으로 현재 코드와 다름)

**현재 알고리즘 — 픽업6 (2026-09 백테스트로 전면 교체):**

```
예상 점유율 = 현재 OTB + mean( 최종점유율_h − 같은시점OTB_h )
              h = 대상월 직전 6개 달력월 중 완료됐고 예약 ≥3건인 달
클램프: [OTB, 100] + 진행 중인 달은 물리적 상한
```

실데이터 301건 백테스트에서 구방식(페이스+STLY 동적가중+편향보정) MAE 12.8%p → **8.0%p**.

**예측 로직 수정 시 반드시 지킬 것:**
- 검증은 반드시 실제 훅을 실행하는 스크립트로 (`run_hook_check.mjs`, `backtest_hook.mjs`).
  공식을 재현한 별도 스크립트는 만들지 말 것 — 미묘한 차이로 오진한 전례가 있음.
- 한 번에 하나씩만 바꾸고 매번 백테스트로 확인할 것 (동시에 바꾸면 서로 상쇄됨).
- 히스토리 윈도는 "대상월 직전 6개 달력월 ∩ 완료된 달"이다. "최근 완료 6개달"로
  바꾸면 먼 지평 정확도가 크게 나빠진다(백테스트 실측 MAE 22.1 vs 17.7%p).
- 팝오버(`ForecastExplainer.tsx`)는 계산 과정을 그대로 보여준다(OTB + 픽업 = 예상).
  공식을 바꾸면 팝오버 문구도 함께 바꿀 것.

핵심 개념:
- **OTB**: On-The-Books (현재 확정 예약 점유율)
- **잔여픽업**: 어떤 시점의 OTB에서 월말 최종까지 추가로 채워진 폭(%p)
- **τ (tau)**: 예약 도착 속도 상수 (최근 12개월로 자동 계산 — 신뢰도 표시에만 사용)
- **STLY**: Same Time Last Year (전년 동월 실적 — 계산에는 미사용, 팝오버 참고 표시용)
- 오픈 초기(첫 ≥3 예약 발생 월 기준 +3개월)는 ADR 참조(STLY)에서 자동 제외
