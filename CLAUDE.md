...
변경 전에 반드시 **`FORECAST_SYSTEM.md`** 를 먼저 읽을 것. (`FORECAST_ALGORITHM.md`는 초기 설명서로 일부 낡음)

**예측 로직 수정 시 반드시 지킬 것 — 어기면 예측이 조용히 폭주함:**
- 예측 공식은 **두 곳**에 있다: `computeForecastRaw()` 본체와, 편향 계산 루프 안의 인라인 복사본.
  **항상 함께 고칠 것.** 한쪽만 고치면 "옛 공식의 오차로 새 공식을 보정"하게 됨.
