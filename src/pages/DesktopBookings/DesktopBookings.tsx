...
import { Plus, ChevronUp, ChevronDown, ChevronsUpDown, Search, X } from 'lucide-react';
...
const SortIcon = ({ col, active, dir }: { col: SortCol; active: boolean; dir: 'asc' | 'desc' }) => {
  if (!active) return <ChevronsUpDown size={9} className="opacity-25 ml-0.5" />;
  return dir === 'asc'
    ? <ChevronUp size={9} className="text-primary ml-0.5" />
    : <ChevronDown size={9} className="text-primary ml-0.5" />;
};
...
const DesktopBookings = () => {
  ...
  const { bookings, properties, addBooking, updateBooking } = useStore();
  ...
  // ── cell class helpers ────────────────────────────────────────────
  const tdBase    = 'px-2.5 py-2 text-[11px] leading-none whitespace-nowrap';
  const tdEditable = `${tdBase} cursor-pointer hover:bg-accent/20 transition-colors`;
  const tdRO      = `${tdBase} text-muted-foreground/60`;
  const inputCls  = 'w-full text-[11px] bg-card border border-primary/60 rounded-[3px] px-1 py-0.5 outline-none focus:border-primary tabular-nums';
  ...
  const Th = ({
    ...
    return (
      <th
        className={`px-2.5 pb-2.5 pt-0 text-[10px] font-semibold text-muted-foreground/50 tracking-wider select-none whitespace-nowrap ${col ? 'cursor-pointer hover:text-muted-foreground' : ''}`}
        onClick={col ? () => handleSort(col) : undefined}
      >
        <span className={`inline-flex items-center gap-0.5 ${alignCls} w-full`}>
          {label}
  ...
  const newInputCls = 'w-full text-[11px] bg-transparent border border-border/40 rounded-[3px] px-1.5 py-1 outline-none focus:border-primary transition-colors placeholder:text-muted-foreground/30 tabular-nums';
  ...
  return (
    <div className="h-full flex flex-col bg-background overflow-hidden">
      ...
      {/* ── Filter bar ── */}
      <div className="px-5 pb-3 flex items-center gap-2 flex-shrink-0 border-b border-border/40 flex-wrap">
        <select
          className="text-[11px] bg-transparent border border-border/50 rounded-lg px-2.5 py-1.5 text-foreground outline-none cursor-pointer hover:border-border transition-colors"
        ...
        <div className="relative ml-auto">
          ...
          <input
            ...
            className="pl-7 pr-6 py-1.5 text-[11px] bg-transparent border border-border/50 rounded-lg text-foreground placeholder:text-muted-foreground/30 outline-none hover:border-border focus:border-primary transition-colors w-40"
          />
          {fsearch && (
        ...
        <span className="text-[10px] text-muted-foreground/40 whitespace-nowrap pl-1">
      ...
      <div className="flex-1 overflow-auto [scrollbar-width:thin]">
        ...
        >
          <colgroup>
            <col style={{ width: 72 }} />   {/* 연월 */}
            <col style={{ width: 60 }} />   {/* 입실 */}
            <col style={{ width: 60 }} />   {/* 퇴실 */}
            <col style={{ width: 42 }} />   {/* 박수 */}
            <col style={{ width: 108 }} />  {/* 예약자명 */}
            <col style={{ width: 40 }} />   {/* 인원 */}
            <col style={{ width: 76 }} />   {/* 채널 */}
            <col style={{ width: 84 }} />   {/* 결제금액 */}
            <col style={{ width: 52 }} />   {/* 수수료율 */}
            <col style={{ width: 76 }} />   {/* 수수료 */}
            <col style={{ width: 84 }} />   {/* 입금액 */}
            <col style={{ width: 72 }} />   {/* ADR */}
            <col style={{ width: 54 }} />   {/* 리드타임 */}
            <col style={{ width: 72 }} />   {/* 예약일 */}
          </colgroup>
          ...
          <tbody>
            ...
            {rows.length === 0 && (
              <tr>
                <td colSpan={14} className="py-20 text-center text-[12px] text-muted-foreground/30">
            ...
            {rows.map((b, idx) => {
              ...
              return (
                <tr
                  key={b.id}
                  className={`border-b border-border/20 hover:bg-accent/10 transition-colors ${idx % 2 !== 0 ? 'bg-muted/[0.03]' : ''}`}
                >
                  {/* 연월 — read-only */}
                  <td className={`${tdRO} text-[10px]`}>{fmtYM(b.checkIn)}</td>
                  ...
                  {/* 예약자명 */}
                  {renderEditCell(b, 'guestName',
                    <span className="font-medium text-foreground">{b.guestName || '—'}</span>,
                    b.guestName, 'text',
                  )}

                  {/* 인원 */}
                  {renderEditCell(b, 'guests',
                    <span className="text-foreground/70">{b.guests}</span>,
                    String(b.guests), 'number', 'center',
                  )}

                  {/* 채널 */}
                  {renderEditCell(b, 'channel',
                    <ChDot ch={b.channel} />,
                    b.channel, 'select',
                  )}

                  {/* 결제금액 */}
                  {renderEditCell(b, 'amount',
                    <span className="font-medium text-foreground tabular-nums">{fmtN(b.amount)}</span>,
                    String(b.amount), 'number', 'right',
                  )}

                  {/* 수수료율 */}
                  {renderEditCell(b, 'commRate',
                    <span className="text-muted-foreground tabular-nums">{commRate}%</span>,
                    commRate, 'number', 'right',
                  )}

                  {/* 수수료 */}
                  {renderEditCell(b, 'commission',
                    <span className="text-destructive/60 tabular-nums">-{fmtN(b.commission)}</span>,
                    String(b.commission), 'number', 'right',
                  )}
                  ...
                  {/* 리드타임 — read-only */}
                  <td className={`${tdRO} text-center tabular-nums`}>
                    {leadTime !== null ? `${leadTime}일` : '—'}
                  </td>

                  {/* 예약일 */}
                  {renderEditCell(b, 'bookingDate',
                    <span className="text-muted-foreground/60">{b.bookingDate ? fmtMD(b.bookingDate) : '—'}</span>,
                    b.bookingDate ?? '', 'date',
                  )}
                </tr>
              );
            ...
            {rows.length > 0 && (
              <tr className="sticky bottom-0 border-t border-border/60 bg-background">
                <td colSpan={3} className={`${tdBase} text-[10px] font-bold text-muted-foreground/50 uppercase tracking-wider`}>
            ...
            {adding && (
              <tr className="border-t-2 border-primary/20 bg-primary/[0.03]">
                {/* 연월 — auto */}
                <td className={`${tdRO} text-[10px]`}>
                  {draft.checkIn ? fmtYM(draft.checkIn) : '—'}
                </td>
                {/* 입실 */}
                <td className="p-1">
                  <input type="date" className={newInputCls} value={draft.checkIn}
                    onChange={e => setDraft(d => ({ ...d, checkIn: e.target.value }))} />
                </td>
                {/* 퇴실 */}
                <td className="p-1">
                  <input type="date" className={newInputCls} value={draft.checkOut}
                    onChange={e => setDraft(d => ({ ...d, checkOut: e.target.value }))} />
                </td>
                {/* 박수 */}
                <td className={`${tdRO} text-center`}>
                  {draftNights > 0 ? `${draftNights}박` : '—'}
                </td>
                {/* 예약자명 */}
                <td className="p-1">
                  <input type="text" placeholder={ko ? '예약자명' : 'Guest name'} className={newInputCls}
                    value={draft.guestName}
                    onChange={e => setDraft(d => ({ ...d, guestName: e.target.value }))} />
                </td>
                {/* 인원 */}
                <td className="p-1">
                  <input type="number" min={1} className={`${newInputCls} text-center`} value={draft.guests}
                    onChange={e => setDraft(d => ({ ...d, guests: e.target.value }))} />
                </td>
                {/* 채널 */}
                <td className="p-1">
                  <select className={newInputCls} value={draft.channel}
                ...
                {/* 결제금액 */}
                <td className="p-1">
                  <input type="number" placeholder="0" className={`${newInputCls} text-right`} value={draft.amount}
                    onChange={e => setDraft(d => ({ ...d, amount: e.target.value }))} />
                </td>
                {/* 수수료율 */}
                <td className="p-1">
                  <input type="number" placeholder="%" className={`${newInputCls} text-right`} value={draft.commRate}
                    onChange={e => setDraft(d => ({ ...d, commRate: e.target.value }))} />
                </td>
                {/* 수수료 — auto */}
                <td className={`${tdBase} text-right text-destructive/60 tabular-nums`}>
                  {draftAmount > 0 ? `-${fmtN(draftCommission)}` : '—'}
                </td>
                {/* 입금액 — auto */}
                <td className={`${tdBase} text-right font-semibold text-primary tabular-nums`}>
                  {draftAmount > 0 ? fmtN(draftNet) : '—'}
                </td>
                {/* ADR — auto */}
                <td className={`${tdRO} text-right tabular-nums`}>
                  {draftADR > 0 ? fmtN(draftADR) : '—'}
                </td>
                {/* 리드타임 — auto */}
                <td className={`${tdRO} text-center tabular-nums`}>
                  {draftLeadTime !== null ? `${draftLeadTime}일` : '—'}
                </td>
                {/* 예약일 */}
                <td className="p-1">
                  <input type="date" className={newInputCls} value={draft.bookingDate}
        ...
        {adding ? (
          <>
            <button
              ...
              className="h-7 px-4 text-[11px] font-semibold bg-primary text-primary-foreground rounded-lg hover:bg-primary-hover disabled:opacity-35 disabled:cursor-not-allowed transition-colors"
            >
              {ko ? '저장' : 'Save'}
            </button>
            <button
              onClick={() => { setAdding(false); setDraft(blankDraft(properties[0]?.id ?? '', today)); }}
          ...
          <button
            ...
            className="h-7 px-3 flex items-center gap-1.5 text-[11px] font-medium text-muted-foreground/60 hover:text-primary border border-dashed border-border/50 hover:border-primary/40 rounded-lg transition-colors"
          >
            <Plus size={12} />
